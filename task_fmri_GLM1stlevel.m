function task_fmri_GLM1stlevel(sub_id,task)
...
fname    = '^swafun.*.nii';                   % filename of func. imgs  'd' refers to despiked image, remove 'd' for non-despiked
...
FunImg = spm_select('ExtFPList',fullfile(path.FunImg,subject{sub_id}),fname,7:228);
% If chosen, get brain mask
if opt.PreBrainMask==1;         % if true, load brain mask
    path.BrainMask = fullfile(path.nvrmask,'rWholeBrain.nii');
end
% Get nuisance variable regressors
nvr_size1 = 0;
nvr_files1 = spm_select('FPList',fullfile(path.FunImg,subject{sub_id}),sprintf('^%s_rp12exp.*.mat',nvrprefix));
...
matlabbatch{2}.spm.stats.con.consess{4}.tcon.name    = sprintf('Negtive Effect of %s',condition(1).name);
matlabbatch{2}.spm.stats.con.consess{4}.tcon.weights = [-1 0 0];
matlabbatch{2}.spm.stats.con.consess{4}.tcon.sessrep = 'none';
matlabbatch{2}.spm.stats.con.consess{5}.tcon.name    = sprintf('Negtive Effect of %s',condition(2).name);
matlabbatch{2}.spm.stats.con.consess{5}.tcon.weights = [0 -1 0];
matlabbatch{2}.spm.stats.con.consess{5}.tcon.sessrep = 'none';
matlabbatch{2}.spm.stats.con.consess{6}.tcon.name    = sprintf('Negtive Effect of %s',condition(3).name);
matlabbatch{2}.spm.stats.con.consess{6}.tcon.weights = [0 0 -1];
matlabbatch{2}.spm.stats.con.consess{6}.tcon.sessrep = 'none';
matlabbatch{2}.spm.stats.con.consess{7}.tcon.name    = sprintf('%s > %s',condition(1).name,condition(2).name);
matlabbatch{2}.spm.stats.con.consess{7}.tcon.weights = [1 -1 0];
matlabbatch{2}.spm.stats.con.consess{7}.tcon.sessrep = 'none';
matlabbatch{2}.spm.stats.con.consess{8}.tcon.name    = sprintf('%s > %s',condition(2).name,condition(1).name);
matlabbatch{2}.spm.stats.con.consess{8}.tcon.weights = [-1 1 0];
matlabbatch{2}.spm.stats.con.consess{8}.tcon.sessrep = 'none';
% F contrasts
matlabbatch{2}.spm.stats.con.consess{9}.fcon.name    = 'Main Effect of Task';
matlabbatch{2}.spm.stats.con.consess{9}.fcon.weights = eye(2);
matlabbatch{2}.spm.stats.con.consess{9}.fcon.sessrep = 'none';
matlabbatch{2}.spm.stats.con.consess{10}.fcon.name    = 'Effects of Interest';
matlabbatch{2}.spm.stats.con.consess{10}.fcon.weights = eye(length(condition));
matlabbatch{2}.spm.stats.con.consess{10}.fcon.sessrep = 'none';
